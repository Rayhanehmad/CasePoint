...
def extract_court_from_citation(citation_text, full_text=None):
    ...
    Removes [ ] brackets from MLD courts.
    ...
    Returns:
        str: Court name (e.g., "Federal Shariat Court", "Lahore") or None if not found
    """
    if not citation_text:
        return None
    ...
    if full_text:
        ...
        if len(lines) >= 2:
            ...
            if mld_bracket:
                return mld_bracket.group(1).strip()
